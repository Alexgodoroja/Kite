...
class ClubModelTestCase(TestCase):
  ...
  fixtures = ["blogs/tests/fixtures/default_club.json", "blogs/tests/fixtures/other_clubs.json"]
  ...
  def club_bio_can_be_520_characters_long(self):
      """Test club bio can be 520 characters long."""
      self.club.bio = "x" * 520
      self._assert_club_is_valid()
      
  def club_bio_cannot_be_over_520_characters_long(self):
      """Test club bio cannot be over 520 characters long."""
      self.club.bio = "x" * 521
      self._assert_club_is_invalid()
      
  def club_name_cannot_contain_numbers(self):
      """Test club cannot contain numbers."""
      self.club.name = "bio 2"
      self._assert_club_is_invalid()
      
  def club_name_cannot_contain_special_characters(self):
      """Test club name cannot contain any special characters."""
      self.club.name = "bio-"
      self._assert_club_is_invalid()
      
  def club_bio_can_be_blank(self):
      """Test club bio can be blank."""
      self.club.bio = ""
      self._assert_club_is_valid()
      
  def club_rules_can_be_blank(self):
      """Test club rules can be blank."""
      self.club.rules = ""
      self._assert_club_is_valid()
      
  def club_name_must_be_unique(self):
